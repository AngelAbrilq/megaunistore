...
function e_mpc(string $v): string { return htmlspecialchars($v, ENT_QUOTES, 'UTF-8'); }
...
<?php if ($flash): ?>
    <div class="alert alert-error"><?= e_mpc($flash['message']) ?></div>
<?php endif; ?>

<div class="form-card">
    <h3>➕ Nuevo Método de Pago</h3>
    <form action="index.php?route=metodos_pago.store" method="POST" id="mpCreateForm">
        <input type="hidden" name="csrf_token" value="<?= e_mpc($csrfToken) ?>">

        <div class="field">
            <label for="mp_nombre">Nombre <span style="color:#dc2626">*</span></label>
            <input type="text" id="mp_nombre" name="nombre" maxlength="100"
                   placeholder="Ej: Nequi, PSE, Criptomonedas…" required>
        </div>

        <div class="field">
            <label for="mp_desc">Descripción</label>
            <textarea id="mp_desc" name="descripcion" rows="3"
...
<script>
document.getElementById('mpCreateForm').addEventListener('submit', function(e) {
    e.preventDefault();
