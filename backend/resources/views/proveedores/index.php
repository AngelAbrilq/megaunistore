...
<div class="mod-topbar">
    ...
    <button class="btn btn-primary" onclick="openModal('index.php?route=proveedores.create&ajax=1')">+ Nuevo proveedor</button>
</div>

<?php if ($flash !== null): ?>
    <div class="alert alert-<?= $flash['type'] === 'success' ? 'success' : 'error' ?>"><?= e_prov($flash['message']) ?></div>
<?php endif; ?>

<div class="card">
    <?php if (empty($proveedores)): ?>
        <div class="empty">No hay proveedores registrados todavía.</div>
    <?php else: ?>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                <?php foreach ($proveedores as $p): ?>
                    <tr>
                        <td><?= e_prov((string) $p['id']) ?></td>
                        <td><strong><?= e_prov($p['nombre']) ?></strong></td>
                        <td><?= e_prov($p['ruc_nit'] ?? '—') ?></td>
                        <td>
                            <?php if (!empty($p['contacto_nombre'])): ?><?= e_prov($p['contacto_nombre']) ?><br><?php endif; ?>
                            <?php if (!empty($p['email'])): ?><?= e_prov($p['email']) ?><br><?php endif; ?>
                            <?php if (!empty($p['telefono'])): ?><?= e_prov($p['telefono']) ?><?php endif; ?>
                        </td>
                        <td>
                        ...
                        <td>
                            <div class="actions">
                                <button class="btn btn-secondary btn-sm"
                                        onclick="openModal('index.php?route=proveedores.edit&id=<?= e_prov((string) $p['id']) ?>&ajax=1')">
                                    Editar
                                </button>
                                <form action="index.php?route=proveedores.toggle" method="POST" class="form-ajax-action">
                                    <input type="hidden" name="csrf_token" value="<?= e_prov($csrfToken) ?>">
                                    <input type="hidden" name="id" value="<?= e_prov((string) $p['id']) ?>">
                                    <input type="hidden" name="estado_actual" value="<?= e_prov((string) ($p['estado'] ?? 1)) ?>">
                                    <button type="submit" class="btn btn-warning btn-sm">
                                        <?= (int) ($p['estado'] ?? 1) === 1 ? 'Desactivar' : 'Activar' ?>
                                    </button>
                                </form>
                                <form action="index.php?route=proveedores.destroy" method="POST" class="form-ajax-action"
                                      data-confirm="¿Seguro que deseas eliminar este proveedor?">
                                    <input type="hidden" name="csrf_token" value="<?= e_prov($csrfToken) ?>">
                                    <input type="hidden" name="id" value="<?= e_prov((string) $p['id']) ?>">
...
document.querySelectorAll('.form-ajax-action').forEach(function(form) {
    form.addEventListener('submit', function(e) {
        ...
        fetch(form.action, { method:'POST', body:new FormData(form) })
            .then(function() { loadContent('proveedores.index', false); })
