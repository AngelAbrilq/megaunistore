...
function e_rpt_mi(string $value): string
{
    return htmlspecialchars($value, ENT_QUOTES, 'UTF-8');
}

$totalMovimientos = count($movimientos);
$entradas = array_filter($movimientos, fn($r) => $r['tipo'] === 'entrada');
$salidas  = array_filter($movimientos, fn($r) => $r['tipo'] === 'salida');
?>

<style>
.rpt-mi-wrap{max-width:1280px;margin:0 auto;padding:24px 20px}
.rpt-mi-wrap h2{margin:0 0 4px;color:#172554;font-size:22px}
.rpt-mi-wrap p.sub{margin:0 0 20px;color:#6b7280;font-size:14px}
...
<div class="rpt-mi-wrap">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;flex-wrap:wrap;gap:12px">
        <div>
            <h2>Movimientos de Inventario</h2>
            <p class="sub">Historial de entradas, salidas y ajustes de stock en el período seleccionado.</p>
        </div>
        <button class="btn btn-secondary" onclick="loadContent('reportes.index', true)">← Volver a reportes</button>
    </div>

    <div class="card">
        <form id="filtroFormMi">
            <div class="filters">
                <div class="form-group">
                    <label for="mi_fecha_inicio">Fecha inicio</label>
                    <input type="date" id="mi_fecha_inicio" name="fecha_inicio" value="<?= e_rpt_mi($fechaInicio) ?>">
                </div>
                <div class="form-group">
                    <label for="mi_fecha_fin">Fecha fin</label>
                    <input type="date" id="mi_fecha_fin" name="fecha_fin" value="<?= e_rpt_mi($fechaFin) ?>">
                </div>
                <div class="form-group">
                    <label for="mi_tienda_id">Tienda</label>
                    <select id="mi_tienda_id" name="tienda_id">
                        <option value="">Todas las tiendas</option>
                        <?php foreach ($tiendas as $tienda): ?>
                            <?php if ($tienda === null) { continue; } ?>
                            <option value="<?= e_rpt_mi((string) $tienda['id']) ?>" <?= (int)$tienda['id'] === $tiendaId ? 'selected' : '' ?>>
                                <?= e_rpt_mi($tienda['nombre']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
    ...
    <div class="summary-grid">
        ...
        <div class="summary-card">
            ...
            <strong style="color:#065f46"><?= count($entradas) ?></strong>
        </div>
        <div class="summary-card">
            <small>Salidas</small>
            <strong style="color:#991b1b"><?= count($salidas) ?></strong>
        </div>
        <div class="summary-card">
            <small>Otros (ajustes)</small>
            <strong style="color:#92400e"><?= $totalMovimientos - count($entradas) - count($salidas) ?></strong>
        </div>
    </div>

    <div class="card">
        <?php if (empty($movimientos)): ?>
            <p style="text-align:center;color:#6b7280;padding:20px 0">No hay movimientos en el período seleccionado.</p>
        <?php else: ?>
            <table id="tablaMi">
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php foreach ($movimientos as $mov): ?>
                        <tr>
                            <td><?= e_rpt_mi($mov['fecha']) ?></td>
                            <td><?= e_rpt_mi($mov['tienda']) ?></td>
                            <td><strong><?= e_rpt_mi($mov['producto']) ?></strong></td>
                            <td>
                                <span class="badge badge-<?= strtolower($mov['tipo']) === 'entrada' ? 'entrada' : (strtolower($mov['tipo']) === 'salida' ? 'salida' : 'ajuste') ?>">
                                    <?= e_rpt_mi(ucfirst($mov['tipo'])) ?>
                                </span>
                            </td>
                            <td><?= number_format((float)$mov['cantidad'], 2) ?></td>
                            <td><?= e_rpt_mi($mov['motivo'] ?? '—') ?></td>
                            <td><?= $mov['ref_tipo'] ? e_rpt_mi(ucfirst($mov['ref_tipo']) . ' #' . $mov['ref_id']) : '—' ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
            <div class="paginator" id="paginatorMi"></div>
        <?php endif; ?>
    </div>
</div>
...
<script>
(function () {
    const form = document.getElementById('filtroFormMi');
    if (form) {
        form.addEventListener('submit', function (e) {
            e.preventDefault();
            const fi = document.getElementById('mi_fecha_inicio').value;
            const ff = document.getElementById('mi_fecha_fin').value;
            const ti = document.getElementById('mi_tienda_id').value;
            let params = 'reportes.movimientos_inventario&fecha_inicio=' + encodeURIComponent(fi) +
    ...
    const tbody = document.querySelector('#tablaMi tbody');
    if (!tbody) return;
    ...
    function renderPag() {
        const c = document.getElementById('paginatorMi');
        if (!c) return;
        let h = `<button class="pag-btn" onclick="_miPag(${currentPage - 1})" ${currentPage === 1 ? 'disabled' : ''}>‹ Ant</button>`;
        let prev = null;
        for (const p of pages()) {
            if (prev !== null && p - prev > 1) h += '<span class="pag-ellipsis">…</span>';
            h += `<button class="pag-btn ${p === currentPage ? 'active' : ''}" onclick="_miPag(${p})">${p}</button>`;
            prev = p;
        }
        h += `<button class="pag-btn" onclick="_miPag(${currentPage + 1})" ${currentPage === totalPages ? 'disabled' : ''}>Sig ›</button>`;
    ...
    window._miPag = function (p) { renderPage(p); };
    renderPage(1);
})();
</script>
