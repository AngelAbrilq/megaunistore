...
function e_rpt_mp(string $value): string
{
    return htmlspecialchars($value, ENT_QUOTES, 'UTF-8');
}

$totalMonto        = array_sum(array_column($ventas, 'total_monto'));
$totalTransacciones = array_sum(array_column($ventas, 'total_ventas'));
?>

<style>
.rpt-mp-wrap{max-width:1280px;margin:0 auto;padding:24px 20px}
.rpt-mp-wrap h2{margin:0 0 4px;color:#172554;font-size:22px}
.rpt-mp-wrap p.sub{margin:0 0 20px;color:#6b7280;font-size:14px}
...
.bar-wrap{background:#e5e7eb;border-radius:4px;height:8px;margin-top:6px}
.bar-fill{background:#1e3a8a;border-radius:4px;height:8px}
...
<div class="rpt-mp-wrap">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;flex-wrap:wrap;gap:12px">
        <div>
            <h2>Ventas por Método de Pago</h2>
            <p class="sub">Distribución de ingresos según el método de pago utilizado.</p>
        </div>
        <button class="btn btn-secondary" onclick="loadContent('reportes.index', true)">← Volver a reportes</button>
    </div>

    <div class="card">
        <form id="filtroFormMp">
            <div class="filters">
                <div class="form-group">
                    <label for="mp_fecha_inicio">Fecha inicio</label>
                    <input type="date" id="mp_fecha_inicio" name="fecha_inicio" value="<?= e_rpt_mp($fechaInicio) ?>">
                </div>
                <div class="form-group">
                    <label for="mp_fecha_fin">Fecha fin</label>
                    <input type="date" id="mp_fecha_fin" name="fecha_fin" value="<?= e_rpt_mp($fechaFin) ?>">
                </div>
                <div class="form-group">
                    <label for="mp_tienda_id">Tienda</label>
                    <select id="mp_tienda_id" name="tienda_id">
                        <option value="">Todas las tiendas</option>
                        <?php foreach ($tiendas as $tienda): ?>
                            <?php if ($tienda === null) { continue; } ?>
                            <option value="<?= e_rpt_mp((string) $tienda['id']) ?>" <?= (int)$tienda['id'] === $tiendaId ? 'selected' : '' ?>>
                                <?= e_rpt_mp($tienda['nombre']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
    ...
    <div class="card">
        <?php if (empty($ventas)): ?>
            <p style="text-align:center;color:#6b7280;padding:20px 0">No hay datos de pagos en el período seleccionado.</p>
        <?php else: ?>
            <table id="tablaMp">
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php foreach ($ventas as $fila): ?>
                        <?php $pct = $totalMonto > 0 ? ((float)$fila['total_monto'] / $totalMonto * 100) : 0; ?>
                        <tr>
                            <td><strong><?= e_rpt_mp($fila['metodo_pago']) ?></strong></td>
                            <td><?= e_rpt_mp((string) $fila['total_ventas']) ?></td>
                            <td><strong>$<?= number_format((float) $fila['total_monto'], 2) ?></strong></td>
                            <td>
                                <?= number_format($pct, 1) ?>%
                                <div class="bar-wrap"><div class="bar-fill" style="width:<?= number_format($pct, 1) ?>%"></div></div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
            <div class="paginator" id="paginatorMp"></div>
        <?php endif; ?>
    </div>
</div>
...
<script>
(function () {
    const form = document.getElementById('filtroFormMp');
    if (form) {
        form.addEventListener('submit', function (e) {
            e.preventDefault();
            const fi = document.getElementById('mp_fecha_inicio').value;
            const ff = document.getElementById('mp_fecha_fin').value;
            const ti = document.getElementById('mp_tienda_id').value;
            let params = 'reportes.ventas_por_metodo_pago&fecha_inicio=' + encodeURIComponent(fi) +
    ...
    function renderPag() {
        const c = document.getElementById('paginatorMp');
        if (!c) return;
        let h = `<button class="pag-btn" onclick="_mpPag(${currentPage - 1})" ${currentPage === 1 ? 'disabled' : ''}>‹ Ant</button>`;
        let prev = null;
        for (const p of pages()) {
            if (prev !== null && p - prev > 1) h += '<span class="pag-ellipsis">…</span>';
            h += `<button class="pag-btn ${p === currentPage ? 'active' : ''}" onclick="_mpPag(${p})">${p}</button>`;
            prev = p;
        }
        h += `<button class="pag-btn" onclick="_mpPag(${currentPage + 1})" ${currentPage === totalPages ? 'disabled' : ''}>Sig ›</button>`;
    ...
    window._mpPag = function (p) { renderPage(p); };
    renderPage(1);
})();
</script>
