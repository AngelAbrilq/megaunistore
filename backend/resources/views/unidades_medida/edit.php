...
function e_euni(string $v): string { return htmlspecialchars($v, ENT_QUOTES, 'UTF-8'); }
?>
<style>
...
<h2 class="mf-title">Editar unidad de medida</h2>
<p class="mf-subtitle">Actualiza la información de la unidad.</p>

<?php if ($flash !== null): ?>
    <div class="mf-alert"><?= e_euni($flash['message']) ?></div>
<?php endif; ?>

<form id="form-editar-unidad" action="index.php?route=unidades.update" method="POST">
    <input type="hidden" name="csrf_token" value="<?= e_euni($csrfToken) ?>">
    <input type="hidden" name="id" value="<?= e_euni((string) $unidad['id']) ?>">
    ...
    <div class="mf-section">
        ...
        <div class="mf-grid">
            <div class="mf-group">
                ...
                <input type="text" id="eu-nombre" name="nombre" required maxlength="80"
                       value="<?= e_euni($unidad['nombre']) ?>">
            </div>
            <div class="mf-group">
                <label for="eu-simbolo">Símbolo *</label>
                <input type="text" id="eu-simbolo" name="simbolo" required maxlength="10"
                       value="<?= e_euni($unidad['simbolo']) ?>">
            </div>
            <div class="mf-group span2">
                <label for="eu-tipo">Tipo</label>
                <input type="text" id="eu-tipo" name="tipo" maxlength="50"
                       value="<?= e_euni($unidad['tipo'] ?? '') ?>">
            </div>
        </div>
    </div>
