...
function e_uni(string $v): string { return htmlspecialchars($v, ENT_QUOTES, 'UTF-8'); }
?>
<style>
...
<div class="mod-topbar">
    <div>
        ...
        <p>Configura las unidades aplicables a los productos.</p>
    </div>
    <button class="btn btn-primary" onclick="openModal('index.php?route=unidades.create&ajax=1')">+ Nueva unidad</button>
</div>

<?php if ($flash !== null): ?>
    <div class="alert alert-<?= $flash['type'] === 'success' ? 'success' : 'error' ?>"><?= e_uni($flash['message']) ?></div>
<?php endif; ?>

<div class="card">
    <?php if (empty($unidades)): ?>
        <div class="empty">No hay unidades de medida configuradas todavía.</div>
    <?php else: ?>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                <?php foreach ($unidades as $u): ?>
                    <tr>
                        <td><?= e_uni((string) $u['id']) ?></td>
                        <td><strong><?= e_uni($u['nombre']) ?></strong></td>
                        <td><span style="font-family:monospace;background:#f1f5f9;padding:2px 8px;border-radius:6px;"><?= e_uni($u['simbolo']) ?></span></td>
                        <td><?= e_uni($u['tipo'] ?? '—') ?></td>
                        <td>
                            <div class="actions">
                                <button class="btn btn-secondary btn-sm"
                                        onclick="openModal('index.php?route=unidades.edit&id=<?= e_uni((string) $u['id']) ?>&ajax=1')">
                                    Editar
                                </button>
                                <form action="index.php?route=unidades.destroy" method="POST" class="form-ajax-action"
                                      data-confirm="¿Seguro que deseas eliminar esta unidad?">
                                    <input type="hidden" name="csrf_token" value="<?= e_uni($csrfToken) ?>">
                                    <input type="hidden" name="id" value="<?= e_uni((string) $u['id']) ?>">
...
document.querySelectorAll('.form-ajax-action').forEach(function(form) {
    form.addEventListener('submit', function(e) {
        ...
        fetch(form.action, { method:'POST', body:new FormData(form) })
            .then(function() { loadContent('unidades.index', false); })
