...
<div class="mod-topbar">
    <div>
        <h2>👥 Clientes</h2>
        <p>Gestiona el directorio de clientes y su fidelización.</p>
    </div>
    <button class="btn btn-primary" onclick="openModal('index.php?route=clientes.create&ajax=1')">+ Nuevo cliente</button>
</div>

<?php if ($flash !== null): ?>
    <div class="alert alert-<?= $flash['type'] === 'success' ? 'success' : 'error' ?>"><?= e_cli($flash['message']) ?></div>
<?php endif; ?>

<div class="card">
    <?php if (empty($clientes)): ?>
        <div class="empty">No hay clientes registrados todavía.</div>
    <?php else: ?>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                <?php foreach ($clientes as $c): ?>
                    <tr>
                        <td><?= e_cli((string) $c['id']) ?></td>
                        <td>
                            <strong><?= e_cli($c['nombre']) ?> <?= e_cli($c['apellido'] ?? '') ?></strong>
                        </td>
                        <td>
                            <?php if (!empty($c['tipo_documento'])): ?>
                                <?= e_cli($c['tipo_documento']) ?>: <?= e_cli($c['numero_documento'] ?? '') ?>
                            <?php else: ?>—<?php endif; ?>
                        </td>
                        <td>
                            <?php if (!empty($c['email'])): ?><?= e_cli($c['email']) ?><br><?php endif; ?>
                            <?php if (!empty($c['telefono'])): ?><?= e_cli($c['telefono']) ?><?php endif; ?>
                        </td>
                        <td><?= e_cli((string) ($c['puntos_fidelidad'] ?? 0)) ?></td>
                        <td>
                            <div class="actions">
                                <button class="btn btn-secondary btn-sm"
                                        onclick="openModal('index.php?route=clientes.edit&id=<?= e_cli((string) $c['id']) ?>&ajax=1')">
                                    Editar
                                </button>
                                <form action="index.php?route=clientes.destroy" method="POST" class="form-ajax-action"
                                      data-confirm="¿Seguro que deseas eliminar este cliente?">
...
document.querySelectorAll('.form-ajax-action').forEach(function(form) {
    form.addEventListener('submit', function(e) {
        ...
        fetch(form.action, { method:'POST', body:new FormData(form) })
            .then(function() { loadContent('clientes.index', false); })
