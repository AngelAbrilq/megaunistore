...
function e_clic(string $v): string { return htmlspecialchars($v, ENT_QUOTES, 'UTF-8'); }
?>
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <title>Nuevo Cliente | Mega_Uni_Store</title>
...
<main class="container">
    <h1>Nuevo cliente</h1>
    <p>Completa los datos del cliente. El nombre es obligatorio.</p>

    <?php if ($flash !== null && $flash['type'] === 'error'): ?>
        <div class="alert alert-error"><?= e_clic($flash['message']) ?></div>
    <?php endif; ?>

    <section class="card">
        <form action="index.php?route=clientes.store" method="POST">
            <input type="hidden" name="csrf_token" value="<?= e_clic($csrfToken) ?>">

            <div class="row">
                <div class="form-group">
                    <label for="nombre">Nombre *</label>
                    <input type="text" id="nombre" name="nombre" required maxlength="100"
                           value="<?= e_clic($_POST['nombre'] ?? '') ?>">
                </div>
                <div class="form-group">
                    <label for="apellido">Apellido</label>
                    <input type="text" id="apellido" name="apellido" maxlength="100"
                           value="<?= e_clic($_POST['apellido'] ?? '') ?>">
                </div>
            </div>
            ...
            <div class="row">
                <div class="form-group">
                    ...
                    <select id="tipo_documento" name="tipo_documento">
                        ...
                        <?php foreach (['CC','CE','NIT','TI','PP'] as $tipo): ?>
                            <option value="<?= e_clic($tipo) ?>"
                                <?= ($_POST['tipo_documento'] ?? '') === $tipo ? 'selected' : '' ?>>
                                <?= e_clic($tipo) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="numero_documento" name="numero_documento" maxlength="30"
                           value="<?= e_clic($_POST['numero_documento'] ?? '') ?>">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="direccion" name="direccion" maxlength="255"
                       value="<?= e_clic($_POST['direccion'] ?? '') ?>">
            </div>

            <div class="form-group">
                <label for="tienda_id">Asociar a tienda</label>
                <select id="tienda_id" name="tienda_id">
                    <option value="">-- Sin tienda especifica --</option>
                    <?php foreach ($tiendas as $t): ?>
                        <option value="<?= e_clic((string) $t['id']) ?>"
                            <?= ((int) ($_POST['tienda_id'] ?? 0)) === (int) $t['id'] ? 'selected' : '' ?>>
                            <?= e_clic($t['nombre']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                <small>Si seleccionas una tienda, el cliente quedara asociado a ella con 0 puntos de fidelidad.</small>
            </div>
